...
const Dashboard = () => {
  ...
  const buttons = [
    { path: "/ai-checkup", label: "AI Checkup", icon: "checkup" },
    { path: "/doctor-appointment", label: "Doctor's Appointment", icon: "doc" },
    { path: "/dispensary", label: "Dispensary", icon: "dis" },
    { path: "/query-bot", label: "AI ChatBot", icon: "bot" },
  ];
          ...
          >
            <Button
              ...
              label={
                <>
                  <img 
                    src={`./src/assets/${hovered === btn.icon ? btn.icon + "_select" : selected === btn.icon ? btn.icon + "_select" : btn.icon}.svg`} 
                    className="w-22 h-22" 
                    alt={btn.label} 
                  />
                  <span className="text-left">{btn.label}</span>
                  <img 
                    src={`./src/assets/${hovered === btn.icon ? "next_select" : selected === btn.icon ? "next_select" : "next"}.svg`} 
